...
6. DUSU – Master Prompt Architect
Specialistici	7. DeepReasoner
8. AI Fundamentals Coach
9. Honesty Extractor
10. Prompt Ingegnere Software
11. (già incluso DUSU come #6)
...
Flusso operativo	1. Copia l’intero prompt fino a “Ora, dimmi il tuo compito”.
2. Incollalo all’inizio della chat.
3. Sostituisci [descrivi qui...] con il tuo task reale.
4. L’AI risponderà con: ruolo, obiettivo, task, contesto, formato, RADAR, sintesi, prossimo passo.
...
Flusso operativo	1. Incolla il prompt in Copilot.
2. Specifica obiettivo e, se presenti, elenca gli altri agenti con nome e competenze.
3. L’AI scompone il task, esegue autonomamente o delega.
4. Alla fine produce un riepilogo azioni.
...
Flusso operativo	1. Copia e incolla il prompt.
2. Sostituisci [INSERIRE IDEA/CONTESTO] con la tua idea.
3. Aggiungi “Inizia dalla Fase 1.”
4. Rispondi alle domande della Fase 1.
5. L’AI procede automaticamente alla Fase 2, 3, 4.
...
Flusso operativo	1. Incolla il prompt.
2. Descrivi il problema (anche vago).
3. L’AI risponde con “Fase 1: Chiarimenti”.
4. Rispondi punto per punto.
5. L’AI passa alla Fase 2 e poi aspetta il tuo consenso per la Fase 3 (“procedi alla Fase 3”).
...
Flusso operativo	1. Copia il prompt dalla prima riga “Agisci come un esperto...” fino a “Ora, dimmi il tuo compito”.
2. Incolla all’inizio della chat.
3. Scrivi il tuo compito (anche dopo).
4. L’AI applica automaticamente i 5 hack.
...
Flusso operativo	1. Incolla l’intero prompt DUSU.
2. Scrivi il tuo task (es. “Crea un prompt per un coach di vendite”).
3. DUSU esegue il doppio audit iniziale, sceglie il livello, applica F.U.G.P. e produce il prompt ottimale.
...
Flusso operativo	1. Copia tutto il prompt (da [RUOLO] alla fine).
2. Incolla nella chat.
3. Scrivi la tua domanda.
4. L’AI esegue l’autovalutazione interna (invisibile) e produce solo l’output XML finale.
...
Flusso operativo	1. Incolla il prompt (da [RUOLO] a “Ora attendi la richiesta”).
2. Fai una domanda su un fondamentale (es. “Aiutami a scrivere un prompt per un reel”).
3. L’AI risponde applicando la regola corrispondente.
...
Flusso operativo	1. Copia il prompt in una chat nuova.
2. Allega il documento (o incolla il testo).
3. Specifica i campi da estrarre (es. “nome cliente, data, importo”).
4. L’AI produce una tabella con colonne Campo | Valore | Fonte | Evidenza | Motivo vuoto.
5. Aggiunge la sezione Audit con livelli ALTO/MEDIO/BASSO.
...
Flusso operativo	1. Incolla il prompt all’inizio della chat.
2. Scrivi la domanda tecnica specificando il linguaggio (es. “In Python, leggi CSV da 10 GB senza esaurire RAM”).
3. L’AI risponde con le 4 sezioni obbligatorie.
...
Honesty Extractor + DUSU	Estrai dati da un documento e poi fai generare a DUSU un prompt per usarli.	1. Honesty Extractor su contratto → ottieni tabella pulita.
2. Incolla la tabella come contesto in DUSU e chiedi: “Crea un prompt per un agente che analizzi questi dati”.
Problem Solver + DeepReasoner	Risolvi problemi complessi con doppia profondità.	1. Problem Solver produce il piano finale (Fase 4).
2. Per ogni fase del piano, chiedi a DeepReasoner: “Applica ragionamento approfondito a questo punto del piano”.
AI Fundamentals Coach + Prompt Ingegnere Software	Impara il fondamentale “vibe coding” e poi applica subito un prompt tecnico.	1. Chiedi al Coach: “Spiegami il vibe coding con un esempio di calcolatrice”.
2. Poi usa Prompt Ingegnere Software: “Genera il codice per quella calcolatrice in React”.
Framework Universale + Agente Autonomo	Struttura un task complesso e poi lo esegue in autonomia.	1. Framework Universale produce un piano dettagliato con livelli e RADAR.
2. Copia la sezione “Piano d’azione” e incollala in Agente Autonomo dicendo: “Esegui questo piano senza chiedere conferme”.
💡 CHICCHE GENERALI (pratiche trasversali)
Prima di usare un prompt, leggi sempre la sezione “Verifica funzionamento” – così sai subito se l’AI lo ha applicato correttamente.

Usa il livello “Rapido” di DUSU o del Framework Universale per domande semplici – non attivare mai l’Approfondito se non serve.

Salva i prompt migliori come “Prompt di sistema” nelle chat che usi spesso (es. Honesty Extractor per l’estrazione dati quotidiana).

Se un prompt produce una risposta troppo lunga o troppo corta, modifica il “Formato di output” direttamente nella copia che usi – aggiungi “max 300 parole” o “risposta in 5 punti”.

La combo più potente per report aziendali:

Honesty Extractor → estrae i numeri reali da documenti.

DeepReasoner → analizza i trend e le cause.

DUSU → genera un prompt per automatizzare l’intero flusso la prossima volta.

Tutti i prompt che contengono le “Leggi IA” (DUSU, Framework Universale, Honesty Extractor) sono tra loro compatibili – puoi unirli tagliando e incollando le sezioni. Usa come base DUSU e innesta Honesty Extractor per l’estrazione.
